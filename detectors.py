...
def is_activation_pose(hand_data):
    ...
    others_extended = (
        hand_data.index_to_wrist_dist > hand_data.palm_size * 1.5 and
        hand_data.middle_to_wrist_dist > hand_data.palm_size * 1.5 and
        hand_data.pinky_to_wrist_dist > hand_data.palm_size * 1.2 # Pinky is naturally shorter
    )
...
def is_volume_pose(hand_data):
    ...
    ratios = np.array([d_ti, d_tm, d_tr, d_tp], dtype=float) / max(1.0, hand_data.palm_size)
    ...
    v_index = np.array([hand_data.index_tip[0] - index_mcp_x, 
                        hand_data.index_tip[1] - index_mcp_y], dtype=float)
    v_thumb = np.array([thumb_x - index_mcp_x, 
                        thumb_y - index_mcp_y], dtype=float)
    ...
    vertical_ok = (thumb_y > (index_mcp_y + config.VOLUME_MIN_VERTICAL_DIFF * hand_data.palm_size))
    horizontal_ok = (abs(thumb_x - index_mcp_x) <= config.VOLUME_HORIZONTAL_TOL * hand_data.palm_size)
...
def is_fist(hand_data):
    ...
    return (
        hand_data.index_to_wrist_dist < curl_threshold and
        hand_data.middle_to_wrist_dist < curl_threshold and
        hand_data.ring_to_wrist_dist < curl_threshold and
        hand_data.pinky_to_wrist_dist < curl_threshold
    )

def get_pinch_distance(hand_data, finger_tip):
    """Returns distance between thumb and specified finger tip."""
    return np.hypot(finger_tip[0] - hand_data.thumb_tip[0], 
                    finger_tip[1] - hand_data.thumb_tip[1])
...
def is_right_click(hand_data):
    ...
    return dist < config.RIGHT_CLICK_DISTANCE_RATIO * hand_data.palm_size
...
def is_mic_mute(hand_data):
    ...
    threshold = config.MIC_MUTE_DISTANCE_RATIO * hand_data.palm_size
