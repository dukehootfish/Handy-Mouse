...
class HandyMouseApp:
    ...
    def _process_activation_hold(self, current_time, hand_data):
        ...
        if not self.activation_pending:
            ...
            # Check stillness
            move_dist = np.hypot(wrist_x - self.activation_anchor_x, 
                               wrist_y - self.activation_anchor_y)
            allowed_wiggle = max(config.TOGGLE_ON_WIGGLE_MIN_PX, 
            ...
            if (current_time - self.activation_start_time) >= config.TOGGLE_ON_STILLNESS_SECONDS:
        ...
        if is_pose:
            ...
            if self.volume_active:
                if self.volume_pose_last_seen_time is None:
                     self.volume_pose_last_seen_time = current_time
                elif current_time - self.volume_pose_last_seen_time > config.VOLUME_EXIT_LEEWAY:
                     self.volume_active = False
                     self.volume_pose_last_seen_time = None

        if not self.volume_active and self.volume_confirm_count >= config.VOLUME_ENTER_CONFIRM_FRAMES:
    ...
    def _process_volume_control(self, img, hand_data):
        ...
        if self.volume_percent_applied is None:
            ...
            self.volume_percent_applied = (1.0 - alpha) * self.volume_percent_applied + \
                                        alpha * self.volume_percent_current
    ...
    def _process_scroll(self, img, hand_data):
        ...
        if self.scroll_origin_x is not None:
            ...
            cv2.putText(img, "Scroll: ON", (40, 550), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 255), 2)
    ...
    def handle_cursor_mode(self, img, hand_data):
        ...
        self.mouse_location = smooth_position(current_raw, self.mouse_location, config.SMOOTHING_FACTOR)
        ...
        if is_mute:
            ...
            cv2.putText(img, "Mic Toggled", (hand_data.img_w // 2 - 100, hand_data.img_h // 2),
    ...
    def _handle_clicks(self, img, hand_data):
        ...
        cv2.putText(img, f'Left Click: {self.mouse_ctrl.left_pressed}', (40, 450), 
                    cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 0), 2)
        cv2.putText(img, f'Right Click: {self.mouse_ctrl.right_pressed}', (40, 500), 
                    cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 0), 2)
    ...
    def _draw_status(self, img):
        ...
        if (not self.system_active) and self.activation_pending and (self.activation_start_time is not None):
            remaining = max(0.0, config.TOGGLE_ON_STILLNESS_SECONDS - (time.time() - self.activation_start_time))
